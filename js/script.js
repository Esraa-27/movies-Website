...
let WordSearch=document.getElementById("localSearch");
let tempWordSearch;
let searchArr=[];
 
 function getSearchWord() {
    tempWordSearch = WordSearch.value;
    let j=-1;

    if(tempWordSearch!=""|| typeof tempWordSearch !="string"){
        for(var i=0; i < arrData.length; i++)
        {
        if(arrData[i].original_title.toLowerCase().includes(tempWordSearch.toLowerCase()))
        {
            searchArr[++j]=arrData[i];
        } 
        }
        arrData=searchArr;
